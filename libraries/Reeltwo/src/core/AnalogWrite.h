    ...
    if (channelNum == -1)
    {
        for (int i = SizeOfArray(_analog_write_channels)-1; i >= 0; i--)
        {
            ...
            if (channel.pin == 0)
            {
                ...
                // printf("ATTACH PIN=%d frequency=%u bits=%d\n", pin, channel.frequency, channel.resolution);
                ledcAttach(pin, ANALOG_WRITE_DEFAULT_FREQUENCY, ANALOG_WRITE_DEFAULT_RESOLUTION);
                if (ledcChangeFrequency(pin, channel.frequency, channel.resolution) != channel.frequency)
                {
...
void analogWriteFrequencyResolution(uint32_t frequency, uint8_t resolution = 8)
{
    for (uint8_t i = 0; i < SizeOfArray(_analog_write_channels); i++)
    {
        ...
        analog_write_channel& channel = _analog_write_channels[i];
        if (channel.pin != 0)
            updateChannel = (ledcChangeFrequency(channel.pin, frequency, resolution) == frequency);
        if (updateChannel)
        {
...
void analogWriteFrequency(uint32_t frequency)
{
    for (uint8_t i = 0; i < SizeOfArray(_analog_write_channels); i++)
    {
        ...
        analog_write_channel& channel = _analog_write_channels[i];
        if (channel.pin != 0)
            updateChannel = (ledcChangeFrequency(channel.pin, frequency, channel.resolution) == frequency);
        if (updateChannel)
            channel.frequency = frequency;
    }
}
...
void analogWriteResolution(uint8_t resolution)
{
    for (uint8_t i = 0; i < SizeOfArray(_analog_write_channels); i++)
    {
        ...
        analog_write_channel& channel = _analog_write_channels[i];
        if (channel.pin != 0)
            updateChannel = (ledcChangeFrequency(channel.pin, channel.frequency, resolution) == channel.frequency);
        if (updateChannel)
            channel.resolution = resolution;
    }
}
