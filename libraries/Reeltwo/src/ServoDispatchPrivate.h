...
private:
    ...
    static bool _ledcSetupTimerFreq(uint8_t pin, double freq, uint8_t bit_num)
    {
        return ledcAttach(pin, (uint32_t)freq, bit_num);
    }
    ...
    double setup(double freq, uint8_t resolution_bits = 10)
    {
        ...
        {
            ledcDetach(fPin);
            ledcAttach(fPin, (uint32_t)freq, resolution_bits);
            return freq;
        }
        return freq;
    }
    ...
    void attachPin(uint8_t pin)
    {
        ...
        attach(pin);
        ledcAttach(pin, (uint32_t)fFreq, fResolutionBits);
    }
